...
{
  XASSERT(layout > CUBEB_LAYOUT_UNSUPPORTED && layout < CUBEB_LAYOUT_MAX &&
  ...
  static DWORD map[CUBEB_LAYOUT_MAX] = {
    0,                    // CUBEB_LAYOUT_UNSUPPORTED(this won't be used.)
...
frames_to_bytes_before_mix(cubeb_stream * stm, size_t frames)
{
  ...
  XASSERT(stm->output_client); // or use XASSERT(has_output(stm))?
  // The output_mix_params.channels is the actual number of output channels.
  size_t stream_frame_size = stm->output_mix_params.channels * sizeof(float);
...
{
  XASSERT(stream_params->layout != CUBEB_LAYOUT_UNSUPPORTED);
                                 ...
                                 cubeb_stream_params * mix_params)
{
  ...
  if (mix_params->layout == CUBEB_LAYOUT_UNSUPPORTED) {
    LOG("Speakers use the unsupported layout!\n");
    ...
    LOG("Channel counts is different from the layout standard!\n");
  }
                   ...
                   cubeb_state_callback state_callback, void * user_ptr)
{
  ...
  if (input_stream_params) {
    ...
    // Make sure the layout matches to the channels.
