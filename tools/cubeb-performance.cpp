...
void
run_stream(cubeb* ctx, uint32_t latency_frames, stream_id* id,
                        ...
                        vector<std::chrono::duration<double>>& times)
{
  ...
  cubeb* ctx;
  int r = init_backend(&ctx, "Context: performance test");
  assert(r == CUBEB_OK && "Error initializing cubeb stream");
  ...
  for (size_t i = 0; i < number_of_streams; ++i) {
    stream_id* id = &ids[i];
    threads.push_back(std::thread(run_stream, ctx, STREAM_LATENCY,
